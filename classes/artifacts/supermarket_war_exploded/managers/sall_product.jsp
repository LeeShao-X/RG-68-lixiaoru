...
<%@page import="BusinessStatistics.userbean"%>
        ...
        $.ajax(
            {
                ...
                success:function (){
                    //  alert("000000");
                    pages=1;
                    window.location="sall_product.jsp?pages=1";
                },
            });
...
<div style="width:1100px;margin: 0 auto;">
	...
	<div style="margin-left: 500px;height: 50px;">
		<ul class="fenye" style="padding-left: 0px;">
			<% if(Page!=1){%>
			<li><a href="sall_product.jsp?pages=<%=(Page-1)%>">◀</a></li>
			<%}%>
			<% for(int i=0;i<k;i++){ if(str[i]!="..."){%>
			<li ><a href="sall_product.jsp?pages=<%=str[i]%>" id="sall_page_<%=str[i]%>"><%=str[i]%></a></li>
			<%} else{%>
			<li ><%=str[i]%></li>
			<%}}%>
			<% if(k>5&&str[5].equals("...")){%>
