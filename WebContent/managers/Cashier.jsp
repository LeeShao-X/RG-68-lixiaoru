...
<%@page import="cashierPage.userbean"%>
...
<script type=text/javascript >
    $(document).ready(function() {
        ...
        $("#cashier_page_"+spage).css({"color":"#ffffff","background-color":"aqua"})
    });
        ...
        $.ajax(
            {
                ...
                success:function (){
                    //  alert("000000");
                    pages=1;
                    window.location="Cashier.jsp?pages=1";
                },
            });
...
<div style="width:1100px;margin: 0 auto;">
    <div style="width:1100px;margin-top: 100px;margin-left: auto;margin-bottom: 20px">
        <input style="height: 30px" type="text" name="finds" size="30" placeholder="员工编号/姓名">
    ...
    <div>
        <table border="2"bordercolor="black"cellpadding="5"  align="center">
            ...
            <%
                ArrayList array=(ArrayList)session.getAttribute("cashier_List");
                if(array==null)
            ...
            <tr>
                ...
                <td><%=us.getCmoney()%></td>
                <td class="lastCol1">
                    <a href="javascript: if(window.confirm('是否删除编号为<%=us.getCid()%>的员工？')){window.location.href='/cdeleteServlet?Cid=<%=us.getCid()%>'}">删除</a>
                </td>
                <td class="lastCol2">
                    <a href="javascript: if(window.confirm('是否修改编号为<%=us.getCid()%>的员工？')){window.location.href='/managers/Cashier_update.jsp?Cid=<%=us.getCid()%>'}">修改</a>
                </td>
            </tr>
            <% } }
            %>
    ...
    <div style="margin-left: 500px;height: 50px;">
        <ul class="fenye" style="padding-left: 0px;">
            <% if(Page!=1){%>
            <li><a href="Cashier.jsp?pages=<%=(Page-1)%>">◀</a></li>
            <%}%>
            <% for(int i=0;i<k;i++){ if(str[i]!="..."){%>
            <li ><a href="Cashier.jsp?pages=<%=str[i]%>" id="cashier_page_<%=str[i]%>"><%=str[i]%></a></li>
            <%} else{%>
            <li ><%=str[i]%></li>
            <%}}%>
            <% if(k>5&&str[5].equals("...")){%>
