...
<script type=text/javascript >
    $(document).ready(function() {
        ...
        $("#message_page_"+spage).css({"color":"#ffffff","background-color":"aqua"})
    });
        ...
        $.ajax(
            {
                ...
                success:function (){
                    //  alert("000000");
                    pages=1;
                    window.location="message.jsp?pages=1";
                },
            });
...
<div style="width:1100px;margin: 0 auto;">
    <div style="width:1100px;margin-top: 100px;margin-left: auto;margin-bottom: 20px">
        <input style="height: 30px" type="text" name="finds" size="30" placeholder="留言列表信息">
    ...
    <div>
        <table border="2"bordercolor="black"cellpadding="5"  align="center">
            ...
            <th>留言标题</th>
            <th>留言内容</th>
            <th class="lastCol1">操作</th></tr>
            <%
                ArrayList array=(ArrayList)session.getAttribute("message_List");
                if(array==null)
                    ...
                    {
                        messageBean us=(messageBean)array.get(i);
            %>
            ...
            <tr>
                ...
                <td><%=us.getMessage()%></td>
                <td class="lastCol1">
                    <a href="javascript: if(window.confirm('是否删除编号为<%=us.getId()%>的员工留言？')){window.location.href='/mdeleteServlet?mid=<%=us.getId()%>'}">删除</a>
                </td>
            </tr>
            <% } }
            %>
    ...
    <div style="margin-left: 500px;height: 50px;">
        <ul class="fenye" style="padding-left: 0px;">
            <% if(Page!=1){%>
            <li><a href="message.jsp?pages=<%=(Page-1)%>">◀</a></li>
            <%}%>
            <% for(int i=0;i<k;i++){ if(str[i]!="..."){%>
            <li ><a href="message.jsp?pages=<%=str[i]%>" id="message_page_<%=str[i]%>"><%=str[i]%></a></li>
            <%} else{%>
            <li ><%=str[i]%></li>
            <%}}%>
            <% if(k>5&&str[5].equals("...")){%>
